...
const loadData = async () => {
  try {
    const res = await request.get('/base/customer/page', { params: searchForm })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/base/customer', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/base/customer', form)
...
const handleDelete = async (row) => {
  try {
    ...
    await request.delete(`/base/customer/${row.id}`)
