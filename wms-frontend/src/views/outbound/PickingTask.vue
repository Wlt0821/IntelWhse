    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="700px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="任务编号" prop="taskNo">
          <el-input v-model="form.taskNo" placeholder="请输入任务编号" :disabled="isEdit" />
        </el-form-item>
        ...
        <el-form-item label="状态" prop="status">
          <el-radio-group v-model="form.status">
            <el-radio label="PENDING">待拣货</el-radio>
            <el-radio label="PROCESSING">拣货中</el-radio>
...
const loadData = async () => {
  try {
    const res = await request.get('/outbound/picking-task/page', { params: searchForm })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/outbound/picking-task', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/outbound/picking-task', form)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该拣货作业吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await request.delete(`/outbound/picking-task/${row.id}`)
