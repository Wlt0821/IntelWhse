  ...
  <div class="zone-container">
    <el-card>
      ...
      <div class="search-bar">
        <el-form :inline="true" :model="searchForm">
          <el-form-item label="库区名称">
            <el-input v-model="searchForm.zoneName" placeholder="请输入库区名称" clearable />
          </el-form-item>
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="zoneName" label="库区名称" width="180" />
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="库区编码" prop="zoneCode">
          <el-input v-model="form.zoneCode" placeholder="请输入库区编码" :disabled="isEdit" />
        </el-form-item>
        ...
        <el-form-item label="库区类型" prop="zoneType">
          <el-select v-model="form.zoneType" placeholder="请选择库区类型">
            <el-option label="普通库区" value="NORMAL" />
            <el-option label="冷藏库区" value="COLD" />
            <el-option label="恒温库区" value="CONSTANT" />
            <el-option label="危险品库区" value="DANGER" />
          </el-select>
        </el-form-item>
...
const loadData = async () => {
  try {
    const res = await request.get('/base/zone/page', { params: searchForm })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/base/zone', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/base/zone', form)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该库区吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await request.delete(`/base/zone/${row.id}`)
