...
const loadData = async () => {
  try {
    const res = await request.get('/base/supplier/page', { params: searchForm })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/base/supplier', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/base/supplier', form)
...
const handleDelete = async (row) => {
  try {
    ...
    await request.delete(`/base/supplier/${row.id}`)
