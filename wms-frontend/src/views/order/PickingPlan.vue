<template>
  <div class="picking-plan-container">
    <el-card>
      <template #header>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="计划类型" prop="planType">
          <el-select v-model="form.planType" placeholder="请选择计划类型">
            <el-option label="按单拣选" value="BY_ORDER" />
            <el-option label="批量拣选" value="BATCH" />
            <el-option label="波次拣选" value="WAVE" />
          </el-select>
        </el-form-item>
...
const loadData = async () => {
  try {
    ...
    const res = await request.get('/order/picking-plan/page', { params })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/order/picking-plan', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/order/picking-plan', form)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除该拣选计划吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await request.delete(`/order/picking-plan/${row.id}`)
