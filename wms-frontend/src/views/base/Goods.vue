    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="商品编码" prop="goodsCode">
          <el-input v-model="form.goodsCode" placeholder="请输入商品编码" :disabled="isEdit" />
        </el-form-item>
...
const loadData = async () => {
  try {
    const res = await request.get('/base/goods/page', { params: searchForm })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put('/base/goods', form)
      ElMessage.success('修改成功')
    } else {
      await request.post('/base/goods', form)
...
const handleDelete = async (row) => {
  try {
    ...
    await request.delete(`/base/goods/${row.id}`)
